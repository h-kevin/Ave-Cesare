...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
  ...
  if ($_SESSION) {
    ...
    if ($_SESSION['admin'] == 1) {
      ...
      $stmt = $conn->prepare("SELECT u.name, u.surname, o.number, o.location, o.time, o.sum, o.status
        FROM `User` u INNER JOIN `Order` o
        ON u.id = o.usr_id
        WHERE o.status = 'open' OR o.status = 'cooking'
      ...
      try {
        ...
        if (!$stmt->execute()) {
          ...
          $stmt->store_result();
          $stmt->bind_result($name, $surname, $onum, $oloc, $time, $total, $status);
